#!/usr/bin/env bash
# Round 6: rerun every collector and figure script in order (numbers*.tex, tables, CSV/JSON, figures), then the raw snapshot and the paper.
# Order matters: hold6/leak_split6 feed collect_pdks5; designs5 feeds pareto/frontend/software6; pareto feeds fig_pareto.
...
$PY sw/hold_orfs6.py > logs/refresh_hold6.log 2>&1 && echo "hold6 ok" || echo "hold6 FAILED"
$PY sw/leak_split6.py > logs/refresh_leak6.log 2>&1 && echo "leak split6 ok" || echo "leak split6 FAILED"
...
$PY sw/collect_software6.py > logs/refresh_software6.log 2>&1 && echo "software6 ok" || echo "software6 FAILED"
...
[ -f sw/collect_util_sweep6.py ] && { $PY sw/collect_util_sweep6.py > logs/refresh_sweep6.log 2>&1 && echo "util sweep6 ok" || echo "util sweep6 FAILED"; }
...
./sw/snapshot_raw6.sh > logs/refresh_snapshot6.log 2>&1 && echo "snapshot ok" || echo "snapshot FAILED"
