#!/usr/bin/env python3
"""Round 5 (E10): figures of the digital front end (rtl/bmi_fe.v) -> paper/numbers_frontend.tex, results/frontend.json.
...
rep = power("power/out_vcd_fe_v2_gls_5m"); out["power_reported"] = rep; out["power_v1_freerunning"] = power("power/out_vcd_fe_v1_gls_5m")
rc = json.load(open(ROOT / "power/out_vcd_fe_v2_gls_5m/root_clock_correction.json")); out["root_clock_correction"] = rc
...
for line in open(ROOT / "power/out_vcd_fe_v2_gls_5m/power_vcd_by_instance.rpt"):
    f = line.split()
    if len(f) < 5: continue
    try: tot = float(f[3])
