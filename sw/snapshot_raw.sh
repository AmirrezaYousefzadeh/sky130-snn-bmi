...
for plat in ihp-sg13g2 nangate45 asap7; do L="$ORFS/logs/$plat/bmi_snn_min16/base"; P="$ORFS/reports/$plat/bmi_snn_min16/base"; [ -d "$L" ] || continue; mkdir -p "$OUT/pdks/$plat"; cp "$L"/6_report.json "$L"/6_report.log "$L"/1_1_yosys.log "$P"/6_finish.rpt "$P"/synth_stat.txt "$P"/synth_check.txt "$OUT/pdks/$plat/" 2>/dev/null || true; done
