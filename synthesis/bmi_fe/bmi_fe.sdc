# Timing constraints for the digital front end bmi_fe: clk5 (CLOCK_PORT, 200 ns) plus the always-on clk32k (30517.578 ns); the two domains are asynchronous (two-flop synchronizer).
...
# second clock: the 32.768 kHz always-on domain (activity register, bin timer); asynchronous to clk5
create_clock -name clk32k -period 30517.578 [get_ports clk32k]
set_clock_groups -asynchronous -group [get_clocks clk5] -group [get_clocks clk32k]
set_input_delay [expr 30517.578 * 0.2] -clock [get_clocks clk32k] [get_ports spike_pulse*]
