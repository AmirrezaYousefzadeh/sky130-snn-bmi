# Timing constraints for the latch-memory cores bmi_snn_lmem / bmi_snn_lmin (based on OpenLane base.sdc).
...
# Latch-based weight memory: the latch contents are static while the core decodes (writes are only accepted when the
# core is idle and the fetch pipeline is empty), so paths launched by the weight latches are false paths. The write path
# (staging register -> latch, half a cycle through one clock gate per row) and the gating checks stay constrained.
