# Timing constraints for bmi_snn_scmem (based on OpenLane base.sdc).
...
# Register-file weights: written only while the core is idle (protocol), static during decoding. Their outputs are
# launched by the write clock gate; declare that domain a false path so that neither setup nor hold is enforced on
# weight -> fetch-register paths (otherwise the slow-corner insertion delay of a 50k-leaf tree dominates).
if { [llength [get_pins -quiet u_icg_wr/GCLK]] > 0 } {
    create_generated_clock -name wr_clk -source [get_pins u_icg_wr/CLK] -divide_by 1 [get_pins u_icg_wr/GCLK]
    set_false_path -from [get_clocks wr_clk]
    puts "\[INFO] wr_clk generated clock declared; paths from the weight register file are false paths"
}
