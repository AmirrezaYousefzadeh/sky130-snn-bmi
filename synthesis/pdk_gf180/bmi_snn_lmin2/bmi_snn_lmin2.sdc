# Timing constraints for bmi_snn_lmin2 (based on OpenLane base.sdc).
...
# latch-based weight memory: paths launched from the weight latches are false (static while decoding); hold at the latch data
# pins waived (data stable from the opening edge for a full cycle) - same constraints as the sky130 latch cores
set wlatches [all_registers -level_sensitive -cells]
if { [llength $wlatches] > 0 } { set_false_path -from $wlatches; set_false_path -hold -to [all_registers -level_sensitive -data_pins]; puts "\[INFO] [llength $wlatches] weight latches: false paths" }
