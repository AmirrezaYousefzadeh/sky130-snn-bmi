...
- Annotated simulation on the other kits (Icarus): compile `sim/timescale_1ns_1ps.v` first, use the notifier-initialised
  ...
  IHP: `sg13g2_udp.v` + the sequential-stripped `sg13g2_stdcell.v` copy + `sim/ihp_seq_icarus.v`, SDF from OpenSTA (`power/run_write_sdf.sh`).
- Raw reports: `sw/snapshot_raw5.sh` -> `results/raw/round5/`. Run log: `results/run_log_round5.md` (chronological narrative);
  `results/ROUND5_CLOSING.md` (closing tables of every experiment, generated by `sw/closing_round5.py`); `results/POLICY5.md` (every
  utilization attempt, `sw/policy_summary5.py`).
