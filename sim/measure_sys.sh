...
# asynchronous), clock propagation stopped at the clock gates, then the round-5 root-clock correction for the clk5 root network of the
...
set -euo pipefail
ROOT="$(cd "$(dirname "$0")/.." && pwd)"; NB="${1:-500}"; TAG="${2:-sys_sp_gls_5m}"
FE_RUN="${FE_RUN:-$ROOT/synthesis/bmi_fe/runs/bmi_fe_5m}"; CORE_RUN="${CORE_RUN:-$ROOT/synthesis/bmi_snn_sp/runs/bmi_snn_sp_5m}"
FE_NL="$FE_RUN/final/nl/bmi_fe.nl.v"; CORE_NL="$CORE_RUN/final/nl/bmi_snn_sp.nl.v"
...
export DESIGN=bmi_sys TOP=bmi_sys RUN_DIR="$FE_RUN" MACRO_INST=none PERIOD_NS=200 VCD_SCOPE=tb_bmi_sys/u_sys CLK_PORT=clk5 CLK2_PORT=clk32k CLK2_PERIOD=30517.578 CLK_STOP_ICG=1
export NETLIST="$FE_NL $CORE_NL $ROOT/rtl/bmi_sys.v" SPEF="u_fe:$FE_RUN/final/spef/nom/bmi_fe.nom.spef u_core:$CORE_RUN/final/spef/nom/bmi_snn_sp.nom.spef"
