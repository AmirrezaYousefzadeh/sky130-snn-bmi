...
case $P in
  ...
  asap7) RUN=$ORFS/results/asap7/$D/base; NL=$RUN/6_final.v; SPEF=$RUN/6_final.spef
...
run_one() { local tag=$1 nb=$2 gap=$3
  ...
  DESIGN=$D MACRO_INST=none PERIOD_NS=20 RUN_DIR=$RUN LIB_SC="$LIB" NETLIST=$NL SPEF=$SPEF $ROOT/power/run_vcd_power.sh core $ROOT/sim/build_$tag/$tag.vcd > $ROOT/logs/vcd_power_$tag.log 2>&1 || { tail -5 $ROOT/logs/vcd_power_$tag.log; return 1; }
