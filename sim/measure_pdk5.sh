...
#   w5000: round 6 (E2): annotated streamed simulation of the first 5,000 bins of the test block (NanGate45: zero-delay), per-pin power
#         -> sim/build_pdk5_<kit>_<core>_{sdf,func}_w5000, power/out_vcd_<tag>   (the same window as Table 3 of the manuscript)
...
RN="${RUN_NICK:-${D}_5m}"; TS="${TAG_SFX:-}"   # round 6: measure another run of the same core (e.g. the decap-free fill, E3b) under its own output tag
case $C in
  ...
  g32p50) VP=g32p50_; INC=$ROOT/rtl/g32p50; LOAD="" ;;   # round 6 (E4c): H = 32 / 50 % grid core on the other kits
...
case $P in
  ...
  nangate45) RUN=$ORFS/results/nangate45/$RN/base; NL=$RUN/6_final.v; SPEF=$RUN/6_final.spef
  ...
  ihp)   RUN=$ORFS/results/ihp-sg13g2/$RN/base; NL=$RUN/6_final.v; SPEF=$RUN/6_final.spef
  ...
  asap7|asap7sram) TU=1e-12; PERIOD_LIB=200000
         if [[ $P == asap7 ]]; then FL=RVT; RUN=$ORFS/results/asap7/$RN/base; else FL=SRAM; RUN=$ORFS/results/asap7/${RUN_NICK:-${D}_5m_sram}/base; fi   # RUN_NICK (round 6): another run of the same core, e.g. the E8 utilization sweep
...
# Round 6 (E6 finding): CLK_STOP_ICG=1 (set_clock_sense -stop_propagation at the clock-gate outputs) removes the combinational power of
# everything behind the clock gates (activities are converted with the clock that reaches each pin; none arrives). All kit clock-gate
# cells carry clock_gating_integrated_cell in their liberty, so OpenSTA scales the gated subtrees itself: no stop (default 0 now).
export CLK_STOP_ICG="${CLK_STOP_ICG:-0}"
...
if [[ $KIND == w5000 ]]; then
  VECF=$ROOT/sim/vecfull_${VP}indy_20160630_01; [[ -d $VECF ]] || { echo "no full-block vectors $VECF"; exit 1; }
  export SIM_TIMEOUT="${SDF_TIMEOUT:-30h}"
  if [[ $P == nangate45 ]]; then VEC=$VECF sim_run ${T0}_func_w5000 5000 0 nosdf
  else
    if [[ $P == asap7 || $P == asap7sram || $P == ihp ]]; then
      SDF_SRC=$ROOT/sim/build_${T0}_sdf_src/design.sdf; mkdir -p "$(dirname $SDF_SRC)"
      [[ -f "$SDF_SRC" ]] || DESIGN=$D RUN_DIR=$RUN LIB_SC="$LIB" NETLIST=$NL SPEF=$SPEF PERIOD_NS=$PERIOD_LIB SDF_OUT=$SDF_SRC $ROOT/power/run_write_sdf.sh > "$(dirname $SDF_SRC)/write_sdf.log" 2>&1 || tail -3 "$(dirname $SDF_SRC)/write_sdf.log"
    fi
    if [[ -f "$SDF_SRC" ]]; then VEC=$VECF sim_run ${T0}_sdf_w5000 5000 0 sdf; else echo "==== $P: no SDF ($C), annotated 5,000-bin window skipped"; fi
  fi
fi
if [[ $KIND == sta ]]; then   # round 6: OpenSTA re-annotation of the existing toggle files of this kit/core (all kinds and supply corners), no simulation
  for b in $ROOT/sim/build_${T0}_md0_*/toggles.tsv $ROOT/sim/build_${T0}_idle_*/toggles.tsv $ROOT/sim/build_${T0}_func_w5000/toggles.tsv $ROOT/sim/build_${T0}_sdf_w5000/toggles.tsv; do
    [[ -f $b ]] || continue; t=$(basename $(dirname $b)); t=${t#build_}
    [[ -n "$(find $b -mmin -${STA_MIN_AGE:-5})" ]] && { echo "==== $P $C $t: toggle file younger than ${STA_MIN_AGE:-5} min, skipped (still being measured)"; continue; }
    clean() { [[ -z "${STA_FORCE:-}" && -f "$1/sta.log" && -f "$1/power_vcd.rpt" ]] && ! grep -q "CLK_STOP_ICG:" "$1/sta.log"; }   # already annotated without the stop
    clean $ROOT/power/out_vcd_$t || { echo "==== $P $C $t re-annotated ($(date +%H:%M:%S))"; sta_run $b $ROOT/power/out_vcd_$t "$LIB"; }
    if [[ $t == *_md0_full || $t == *_idle_full ]]; then for cv in "${LOWV[@]}"; do corner=${cv%%:*}; libs=${cv#*:}; clean $ROOT/power/out_vcd_${t}_$corner || { echo "==== $P $C $t at $corner re-annotated"; sta_run $b $ROOT/power/out_vcd_${t}_$corner "$libs"; }; done; fi
  done
fi
