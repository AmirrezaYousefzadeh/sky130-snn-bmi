// Top for the parallel core with weights in a standard-cell register file (loaded through the write port).
`timescale 1ns/1ps
module bmi_snn_scmem #(parameter integer H = 64, THETA = 256, K1 = 4, K2 = 4)(
  ...
  bmi_snn_par #(.H(H), .THETA(THETA), .K1(K1), .K2(K2), .HARDWIRED(0)) u_core (
    .clk(clk), .reset(reset), .mode_dense(mode_dense), .ev_valid(ev_valid), .ev_ch(ev_ch), .ev_ready(ev_ready),
