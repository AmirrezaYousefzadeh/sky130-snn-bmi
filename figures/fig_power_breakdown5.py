...
ORDER = [("bmi_snn_top", "SRAM seq."), ("bmi_snn_topg", "SRAM seq. gated"), ("bmi_snn_lmin2", "latch 12-bit"), ("bmi_snn_hw", "hardwired 20-bit"), ("bmi_snn_min", "hw 16-bit"),
...
for name, label in ORDER:
    d = D.get(name)
    if not d or not d.get("event"): continue
    e = d.get("event_sdf") or d["event"]; g = e["groups"]; tot = g["Total"]["total"]
    ...
    items.append(dict(name=name, label=label, E=E, annotated=("event_sdf" in d), parts=parts, leak=leak, leak100=l100, leak37=l37, dyn=E * RATE * 1e-3))
...
for xi, it in zip(x, items): a1.text(xi, 101, f"{it['E']:.2g} nJ" + ("" if it["annotated"] else "*"), ha="center", va="bottom", fontsize=6)
a1.set_xticks(x); a1.set_xticklabels([it["label"] for it in items], rotation=35, ha="right", fontsize=6.5); a1.set_ylabel("share of power while decoding (%)"); a1.set_ylim(0, 112)
...
for xi, it in zip(x, items):
    ...
    a2.text(xi, it["dyn"] + (it["leak"] or 0), f"{it['dyn'] + (it['leak'] or 0):.2g}", ha="center", va="bottom", fontsize=6)
...
fig.tight_layout(); out = ROOT / "paper/figures"; fig.savefig(out / "power_breakdown5.pdf", bbox_inches="tight"); fig.savefig(out / "power_breakdown5.png", dpi=200, bbox_inches="tight")
